            ...
            <div class="list-group-item"> 
                <div class="d-flex">
                    <div class="mr-auto p-2">
                        ...
                        <?php
                        $result_contador = "SELECT *FROM adms_usuarios";
                        $resultado_contador = mysqli_query($conn, $result_contador);
                        $total_contador = mysqli_num_rows($resultado_contador)
                        ?>
                        <h6 class="lead">Listado temos <?php echo "$total_contador"; ?> usuários até o momento.</h6>
                    </div>
                    <div class="p-2">
                        <?php
                        $btn_cad = carregar_btn('cadastrar/cad_usuario', $conn);
                        if ($btn_cad) {
                            echo "<a href='" . pg . "/cadastrar/cad_usuario' class='btn btn-outline-success btn-sm'>Cadastrar usuários</a>";
                        }
                        ?>
                    </div>
                </div>
                ...
                if ($_SESSION['adms_niveis_acesso_id'] == 1) {
                    $resul_user = "SELECT user.id, user.nome, user.usuario, upa.nome_da_unidade,
                        nivac.nome nome_nivac
                        FROM adms_usuarios user
                        INNER JOIN adms_niveis_acessos nivac ON nivac.id=user.adms_niveis_acesso_id
                        INNER JOIN adms_unidades upa ON upa.id=user.adms_unidade_id
                            ...
                            <tbody>
                                ...
                                while ($row_user = mysqli_fetch_assoc($resultado_user)) {
                                    ...
                                    <tr>
                                        ...
                                        <td class="d-none d-sm-table-cell"><?php echo $row_user['usuario']; ?></td>
                                        <td class="d-none d-sm-table-cell"><?php echo $row_user['nome_nivac']; ?></td>
                                        <td class="d-none d-sm-table-cell"><?php echo $row_user['nome_da_unidade']; ?></td>
                                        <td class="text-center">
                                            <span class="d-none d-md-block">
                                                ...
                                                if ($btn_apagar) {
                                                    echo "<a href='" . pg . "/processa/apagar_usuario?id=" . $row_user['id'] . "' class='btn btn-outline-danger btn-sm' data-confirm='VOCÊ TEM CERTEZA QUE QUER EXCLUÍR O ITEM SELECIONADO?'>Apagar</a> ";
                                                }
                                                ?>
                                            </span>
                                            <div class="dropdown d-block d-md-none">
                                                ...
                                                <div class="dropdown-menu dropdown-menu-right" aria-labelledby="acoesListar">
                                                    ...
                                                    if ($btn_apagar) {
                                                        echo "<a class='dropdown-item' href='" . pg . "/processa/apagar_usuario?id=" . $row_user['id'] . "' data-confirm='VOCÊ TEM CERTEZA QUE QUER EXCLUÍR O ITEM SELECIONADO?'>Apagar</a> ";
                                                    }
                                                    ?>
                                                </div>
                                            </div>
