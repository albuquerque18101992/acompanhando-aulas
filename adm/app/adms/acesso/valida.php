    ...
    if ((!empty($usuario)) AND ( !empty($senha))) {
        ...
        if (($resultado_user)AND ( $resultado_user->num_rows != 0)) {
            ...
            if (password_verify($senha, $row_user['senha'])) {
                //Pesquisando a ordem do nível de acesso.
                ...
                $_SESSION['msg_de_erro'] = "<div class='alert alert-danger'>Usuário ou Senha não cadastrados!</div>";
                $url_destino = pg . '/acesso/login';
                header("Location: $url_destino");
            }
        } else {
            $_SESSION['msg_de_erro'] = "<div class='alert alert-danger'>Usuário ou Senha errados!</div>";
            $url_destino = pg . '/acesso/login';
            header("Location: $url_destino");
        }
    } else {
        $_SESSION['msg_de_erro'] = "<div class='alert alert-danger'>Usuário ou Senha incorretos!</div>";
        $url_destino = pg . '/acesso/login';
        header("Location: $url_destino");
    }
} else {
